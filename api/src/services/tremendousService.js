...
class TremendousService {
  ...
  async sendGiftCard({ recipientEmail, recipientName, amount, message, customMessage = null }) {
    try {
      ...
      // Add campaign_id if provided and not a placeholder
      if (this.campaignId && this.campaignId !== 'your_campaign_id_here') {
        giftCardData.reward.campaign_id = this.campaignId;
      }

      // Only add custom_fields if message field ID is provided and not a placeholder
      if (process.env.TREMENDOUS_MESSAGE_FIELD_ID && process.env.TREMENDOUS_MESSAGE_FIELD_ID !== 'your_message_field_id_here') {
        giftCardData.reward.custom_fields = [
          {
      ...
      return { 
        success: true, 
        orderId: order.id,
        giftCardId: reward?.id,
        giftCardCode: reward?.id, // Use reward ID as the code for now
        redeemUrl: null, // Not immediately available, would need to fetch later
        details: order
      };
    } catch (error) {
